...
// --- FIN CALENDARIO ---
// Buscar trabajador por usuario o nombre
...
// Función para restablecer las horas de todos los trabajadores a 0 (accesible globalmente)
function resetearHorasTrabajadores() {
    if (confirm('¿Estás seguro de que quieres restablecer las horas de todos los trabajadores a 0?')) {
        ...
        // Limpiar registros guardados en localStorage
        registroHoras = {};
        horasPorDia = {};
        ...
        // Actualizar vistas si el panel admin está abierto
...
// ----- Aviso: Resumen de condiciones de trabajo (Francia) -----
function showJobSummaryModal() {
...
function closeJobSummaryModal(persist = false) {
    const modal = document.getElementById('jobSummaryModal');
    if (!modal) return;
    modal.classList.remove('show');
    modal.setAttribute('aria-hidden', 'true');
    setTimeout(() => { modal.style.display = 'none'; }, 300);
    // No persistence: el modal volverá a mostrarse en cada recarga
}

document.addEventListener('DOMContentLoaded', function() {
    try {
        // Mostrar en cada carga
        showJobSummaryModal();

        const jobModal = document.getElementById('jobSummaryModal');
        if (jobModal) {
            jobModal.addEventListener('click', function (e) {
                if (e.target === jobModal) closeJobSummaryModal(false);
            });
            const closeBtn = document.getElementById('jobSummaryClose');
            if (closeBtn) closeBtn.addEventListener('click', function () { closeJobSummaryModal(false); });

            const contactBtn = document.getElementById('jobContactBtn');
            if (contactBtn) contactBtn.addEventListener('click', function (e) {
                e.preventDefault();
                closeJobSummaryModal(false);
                const contactSection = document.getElementById('contacto');
                if (contactSection) contactSection.scrollIntoView({ behavior: 'smooth' });
            });
        }
    } catch (err) {
        console.warn('Aviso modal no disponible:', err);
    }
});
